...
import { Budget as BudgetType, BudgetWithSpending } from '@/types';
...
export function Budget() {
  ...
  const loadBudgets = () => {
    ...
    const budgetsWithSpending: BudgetWithSpending[] = budgetData.map((budget) => {
      ...
      const percentage = budget.monthlyBudget > 0 ? (totalSpent / budget.monthlyBudget) * 100 : 0;
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <p className="text-sm text-muted-foreground mt-1">Click "Add Category" to create your first budget category</p>
            </div>
          </CardContent>
        </Card>
      ) : (
              ...
              <CardHeader>
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    ...
                    <Badge className={`mt-2 ${categoryColors[budget.categoryType]}`} variant="outline">
                      {budget.categoryType}
                    </Badge>
                  </div>
              ...
              <CardContent className="space-y-4">
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium">RM {budget.totalSpent.toFixed(2)}</span>
                  </div>
                  <Progress value={Math.min(budget.percentage, 100)} className="h-2" />
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Budget</span>
                    <span className="font-medium">RM {budget.monthlyBudget.toFixed(2)}</span>
                  </div>
                </div>
                <div className="pt-2 border-t">
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Remaining</span>
                    <span className={`font-semibold ${budget.remaining < 0 ? 'text-destructive' : 'text-emerald-500'}`}>
