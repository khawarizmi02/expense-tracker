...
export function MonthForm({ open, onOpenChange, onSuccess, editData }: MonthFormProps) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <form onSubmit={handleSubmit}>
          <DialogHeader>
            <DialogTitle>{editData ? 'Edit' : 'Add'} Month Classification</DialogTitle>
            <DialogDescription>
              Create a month classification to organize your expenses and incomes.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                required
              />
              <p className="text-xs text-muted-foreground">Format: MM-YY (e.g., 01-26)</p>
            </div>
            <Button type="button" variant="outline" size="sm" onClick={generateFromDate}>
              Use Current Month
            </Button>
          </div>
          <DialogFooter>
